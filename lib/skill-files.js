...
export function parseFrontmatter(raw) {
     ...
     */
    const readBlockScalar = (indicator, keyLine) => {
        const keyIndent = /^[ \t]*/.exec(lines[keyLine])[0].length;
        const collected = [];
        for (let i = keyLine + 1; i < lines.length; i++) {
            ...
            if (/^[ \t]*/.exec(line)[0].length <= keyIndent)
                break;
            collected.push(line);
        }
        ...
        const body = collected.map((line) => {
            if (line.trim() === "")
                return "";
            return line.slice(Math.min(blockIndent, /^[ \t]*/.exec(line)[0].length));
        });
 ...
 */
function resolvableEntryName(parsedName, fallback) {
