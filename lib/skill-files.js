        ...
        if (isDir) {
            ...
            if (mdExists || disabledExists) {
                ...
                entries.push({
                    ...
                    description: entryDescription(parsed, parsed?.name ?? item.name),
            ...
            if (item.name.endsWith(".md" + DISABLED_SUFFIX)) {
                ...
                entries.push({ name: parsed?.name ?? item.name.slice(0, -(".md" + DISABLED_SUFFIX).length), description: entryDescription(parsed, parsed?.name ?? item.name.slice(0, -(".md" + DISABLED_SUFFIX).length)), whenToUse: parsed?.whenToUse, enabled: false, kind: "flat", file: full, dirBundle: false, source: root.source, projectRoot: root.projectRoot, rel: "" });
            }
            else if (item.name.endsWith(".md")) {
                const parsed = parseFrontmatter(await readFile(full, "utf8"));
                entries.push({ name: parsed?.name ?? item.name.slice(0, -3), description: entryDescription(parsed, parsed?.name ?? item.name.slice(0, -3)), whenToUse: parsed?.whenToUse, enabled: true, kind: "flat", file: full, dirBundle: false, source: root.source, projectRoot: root.projectRoot, rel: "" });
            }
        }
    }
}
